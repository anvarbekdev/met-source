...
async function seed() {
    ...
    for (const c of clinics) {
        await qr.query(`INSERT INTO clinics (id, name, address, lat, lng, phone, working_hours)
       VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT (id) DO NOTHING`, [c.id, c.name, c.address, c.lat, c.lng, c.phone, c.workingHours]);
    }
    ...
    for (const u of users) {
        await qr.query(`INSERT INTO users (id, full_name, phone, email, password_hash, role, clinic_id)
       VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT (id) DO NOTHING`, [u.id, u.fullName, u.phone, u.email, u.hash, u.role, u.clinicId]);
    }
    ...
    for (const d of departments) {
        await qr.query(`INSERT INTO departments (id, clinic_id, name, type)
       VALUES ($1,$2,$3,$4) ON CONFLICT (id) DO NOTHING`, [d.id, d.clinicId, d.name, d.type]);
    }
    ...
    for (const d of doctors) {
        await qr.query(`INSERT INTO doctors (id, user_id, department_id, specialization, experience_years)
       VALUES ($1,$2,$3,$4,$5) ON CONFLICT (id) DO NOTHING`, [d.id, d.userId, d.deptId, d.spec, d.exp]);
    }
    ...
    for (const p of patients) {
        await qr.query(`INSERT INTO patients (id, user_id, full_name, phone, gender, birth_date, blood_type, address)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT (id) DO NOTHING`, [p.id, p.userId, p.fullName, p.phone, p.gender, p.birth, p.blood, p.address]);
    }
    console.log('✅ Patients (8)');
    const now = new Date();
    const ago = (days) => new Date(now.getTime() - days * 86_400_000);
    const fwd = (days) => new Date(now.getTime() + days * 86_400_000);
    const appointments = [
    ...
    for (const a of appointments) {
        await qr.query(`INSERT INTO appointments (id, patient_id, doctor_id, clinic_id, status, scheduled_at, reason)
       VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6)`, [a.p, a.d, a.c, a.status, a.at, a.reason]);
    }
    ...
    for (const f of onkoFiles) {
        await qr.query(`INSERT INTO medical_files (id, patient_id, uploaded_by_id, file_url, original_name, mime_type, file_size, "fileType", module_source)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) ON CONFLICT (id) DO NOTHING`, [f.mfId, f.patId, f.upId, f.url, f.name, f.mime, f.size, f.fileType, f.moduleSource]);
        const d = f.diagnosis;
        await qr.query(`INSERT INTO diagnosis_results
    ...
    for (const f of digiFiles) {
        await qr.query(`INSERT INTO medical_files (id, patient_id, uploaded_by_id, file_url, original_name, mime_type, file_size, "fileType", module_source)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) ON CONFLICT (id) DO NOTHING`, [f.mfId, f.patId, f.upId, f.url, f.name, f.mime, f.size, 'PDF', 'DOC_DIGITIZER']);
        await qr.query(`INSERT INTO digitized_documents (id, original_file_id, extracted_data_json, raw_text, status)
       VALUES (gen_random_uuid(), $1, $2, $3, $4)`, [f.mfId, JSON.stringify(f.extractedData), f.rawText, f.status]);
    }
